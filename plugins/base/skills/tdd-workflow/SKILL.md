---
name: TDD Workflow
description: This skill should be used when the user asks to "write tests first", "implement TDD", "test-driven development", "unit testing", "integration testing", "E2E testing", "test coverage", or when writing new features, fixing bugs, or refactoring code. Enforces test-driven development with 80%+ coverage.
---
...
Example:
As a user, I want to search for products by description,
...
describe('Product Search', () => {
  ...
  it('falls back to basic search when cache unavailable', async () => {
...
test('user can search and filter items', async ({ page }) => {
  ...
  // Search for items
  await page.fill('input[placeholder="Search items"]', 'laptop')
...
```typescript
jest.mock('@/lib/cache', () => ({
  searchItemsByQuery: jest.fn(() => Promise.resolve([
    { slug: 'test-item', relevance_score: 0.95 }
  ])),
  checkCacheHealth: jest.fn(() => Promise.resolve({ connected: true }))
}))
```
